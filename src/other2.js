export function outsideMethod2() {
  ...
  async function fetchWeatherJSON() {
    const response = await fetch(
      `http://api.weatherapi.com/v1/forecast.json?key=e5fa685f438648b390a181343220302&q=${location}&days=1&aqi=no&alerts=no`
    );
  ...
  void function setForecastDay(day) {
    fetchWeatherJSON().then((obj) => {
      ...
      const moon_illumination =
        obj.forecast.forecastday[day].astro.moon_illumination;
      ...
      const forecast_condition_text =
        obj.forecast.forecastday[day].day.condition.text;
      const daily_chance_of_rain =
        obj.forecast.forecastday[day].day.daily_chance_of_rain;
      const daily_chance_of_snow =
        obj.forecast.forecastday[day].day.daily_chance_of_snow;
      const daily_will_it_rain =
        obj.forecast.forecastday[day].day.daily_will_it_rain;
      const daily_will_it_snow =
        obj.forecast.forecastday[day].day.daily_will_it_snow;
