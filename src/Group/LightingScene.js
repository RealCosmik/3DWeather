...
export async function Initialize(scene, camera, canvas) {
    ...
    rotationData.angleControl = RotationFolder.add(rotationData, 'angle', 0, 360, .5);
    RotationFolder.add(rotationData, 'manualControl', 0, 1, 1);
    const pivotFolder = RotationFolder.addFolder("pivot");
    pivotFolder.add(rotationData.pivot, 'x', -100, 100, .5);
    pivotFolder.add(rotationData.pivot, 'y', -100, 100, .5);
    pivotFolder.add(rotationData.pivot, 'z', -100, 100, .5);
    const originFolder = RotationFolder.addFolder("origin");
    originFolder.add(rotationData.origin, 'x', -100, 100, .5);
    originFolder.add(rotationData.origin, 'y', -100, 100, .5);
    originFolder.add(rotationData.origin, 'z', -100, 100, .5);
    sun.sunModel.position.set(0, 100, 0);
    sun.sunModel.scale.set(.5, .5, .5);
...
function AddLightsToScene(scene, lightFolder) {
    ...
    rotationData.intensityControl = lightFolder.add(rotationData.mainLight, 'intensity', 0, 1, .0055);
...
function RotateAroundPoint(pivot, origin, angle) {
    ...
    const xNew = (xFactor * (origin.x - pivot.x)) - (yFactor * (origin.y - pivot.y)) + pivot.x;
    const yNew = (yFactor * (origin.x - pivot.x)) + (xFactor * (origin.y - pivot.y)) + pivot.y;
...
function GetElaspedHours() {
    ...
    const dateString = WeatherData.current.last_updated.substring(10, WeatherData.current.last_updated.length);
...
function update(deltaTime) {
    ...
    const newPos = RotateAroundPoint(rotationData.pivot, rotationData.origin, rotationData.angle);
